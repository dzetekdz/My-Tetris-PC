...
function sweep(){
  ...
  if(lines > 0){
    ...
    updateHighscore(); // kontrola highscore
    if(score % 600 < 100){ 
      level++;
      dropSpeed *= 0.9;
    }
...
function drawNext(){ 
  nextQueue.forEach((piece,i)=>{ 
    const ctxNext = nextCanvases[i].getContext("2d"); 
    ctxNext.clearRect(0,0,nextCanvases[i].width,nextCanvases[i].height); 
    drawMatrix(piece,{x:0,y:0},1,ctxNext); 
  }); 
}
